...
"""
file_writer.py
Telemetry file writing and parallel file creation
"""
...
class TelemetryFileWriter:
    """Writes telemetry files in various formats"""
    ...
    def write_records_enhanced(
        ...
        *,
        ...
        """Write records in various formats"""
        try:
            if start_seq_id is not None:
                generator._next_seq_id = start_seq_id
            
            # Default record type ratio
            if record_type_ratio is None:
                record_type_ratio = {RecordType.UPDATE: 0.7, RecordType.EVENT: 0.3}
            ...
            try:
                ...
                self.logger.error(f"Failed to create directory for {path}: {e}")
                raise
            ...
            try:
                with open(path, mode, encoding=encoding) as f:
                    ...
                    for i in range(num_records):
                        try:
                            # Select record type based on ratio
                            rand = random.random()
                            cumulative = 0
                            selected_type = RecordType.UPDATE
                            ...
                            if use_ndjson:
                                try:
                                    ...
                                    self.logger.error(f"Error writing NDJSON record {i}: {e}")
                                    continue
                                ...
                                try:
                                    ...
                                    self.logger.error(f"Error packing binary record {i}: {e}")
                                    continue
                                ...
                                try:
                                    ...
                                    self.logger.error(f"Error writing InfluxDB line record {i}: {e}")
                                    continue
                                ...
                                try:
                                    ...
                                    self.logger.error(f"Error preparing JSON record {i}: {e}")
                                    continue
                            ...
                            if progress_callback and i % 1000 == 0:
                                try:
                                    ...
                                    self.logger.warning(f"Progress callback error at record {i}: {e}")
                                    
                        except Exception as e:
                            self.logger.error(f"Error generating record {i}: {e}")
                            continue
                    ...
                    if output_format == OutputFormat.JSON and not use_ndjson:
                        try:
                            ...
                            self.logger.error(f"Error writing JSON array to file: {e}")
                            raise
                ...
                self.logger.error(f"Unexpected error writing to {path}: {e}")
                raise
    ...
    def generate_multiple_files_enhanced(
        ...
        *,
        ...
        """Generate multiple files with advanced features"""
        paths: List[str] = []
        futures = []
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as ex:
                for i in range(num_files):
                    try:
                        ...
                        self.logger.error(f"Error submitting file generation task {i}: {e}")
                        continue
            ...
            self.logger.error(f"Error in generate_multiple_files_enhanced: {e}")
            raise
    ...
    def _write_enhanced_file_isolated(
        ...
        """Write advanced file in isolated mode"""
        try:
            # Create isolated instance that loads from dict
            try:
                isolated = self.generator_class(
                    schema_dict=self.schema,
                    output_dir=os.path.dirname(path) or ".",
                    default_record_type=default_record_type,
                    enable_gpu=False,  # No GPU in isolated threads
                    logger=self.logger
                )
            except Exception as e:
                self.logger.error(f"Error creating isolated generator for {path}: {e}")
                raise
            
            try:
                isolated._next_seq_id = file_start_seq
            except AttributeError as e:
                self.logger.warning(f"Could not set sequence ID for isolated generator: {e}")
    ...
    async def generate_multiple_files_enhanced_async(
        ...
        *,
        ...
        """Async wrapper for multiple file generation"""
        try:
            loop = asyncio.get_running_loop()
            return await loop.run_in_executor(
                None,
            ...
            self.logger.error(f"Error in async file generation: {e}")
            raise